# if the following packages are not in your computer, they will be automatically installed using the following function

list.of.packages <- c("SCI", "fitdistrplus","extRemes", "stringr","lubridate", "SPEI","lmomco", "evd","evd", "dplyr",
                      "mev", "goftest","zoo", "evd","EnvStats", "Metrics","car","rstudioapi","fs")
...
dgext <- function(x, shape, scale, location) {
  e <- location
  a <- scale
  k <- shape
...
pgext <- function(q, shape, scale, location) {
  e <- location
  a <- scale
  k <- shape
...
qgext <- function(p, shape, scale, location) {
  e <- location
  a <- scale
  k <- shape
  p.correct <- !is.na(p)
  x <- vector()
  if(k==0) {
    x[p.correct] <- e-a*log(-log(p[p.correct]))
  } else {
    x[p.correct] <- e+(a/k)*(1-((-log(p[p.correct]))^k))
  }
  x[is.na(p)] <- NA
  return(x)
}
